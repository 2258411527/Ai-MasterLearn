<template>
  <el-dialog :title="title" :visible.sync="visible" width="60%">
    <el-form :model="noticeForm" label-width="80px">
      ...
      <el-form-item v-if="!isAdmin" label="是否公开">
        <el-switch
      ...
      <el-form-item v-if="!noticeForm.isPublic && !isAdmin" label="公告班级">
        <ClassSelect
      ...
      <el-form-item v-if="isAdmin" label="公告范围">
        <el-tag type="success" size="medium">系统公告（全部公开）</el-tag>
      </el-form-item>
      <el-form-item label="必读公告">
        <el-switch
          v-model="noticeForm.isMustRead"
          :disabled="!isEdit"
          active-color="#f56c6c"
      ...
      <el-form-item label="消息类型">
        <el-select v-model="noticeForm.noticeType" :disabled="!isEdit" placeholder="请选择消息类型">
...
export default {
  ...
  created() {
    const role = getRole()
    this.isAdmin = role === 3
    if (this.isAdmin && this.editType === 1) {
      this.noticeForm.isPublic = true
      this.roleEdit = true
    }
  },
