        ...
        <el-card class="qu-content content-h">
          ...
          <div v-if="quData.quType === 4">
            <el-input
              ...
              type="textarea"
              :autosize="{ minRows: 2 }"
              placeholder="请输入内容（最多5000字）"
              show-word-limit
              :maxlength="5000"
              @input="checkSaqLength"
            />
            <div style="font-size: 12px; color: #909399; margin-top: 5px;">
              最多可输入5000字，当前已输入 {{ saqTextarea ? saqTextarea.length : 0 }} 字
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    // 检查简答题长度
    checkSaqLength() {
      if (this.saqTextarea && this.saqTextarea.length > 5000) {
        this.$message.warning('简答题答案长度超过限制，已自动截断前5000字')
        this.saqTextarea = this.saqTextarea.substring(0, 5000)
      }
    },
