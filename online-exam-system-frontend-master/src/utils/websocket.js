...
// 定义 WebSocket 实例
let socket
const baseSocketUrl = 'ws://localhost:8080/api/websocket'
...
function sendMessage(message) {
  ...
  if (!socket) {
    ...
    console.error('WebSocket 未初始化，尝试重新连接,请稍后再试')
    connectWebSocket()
    return false
  }
  ...
  if (readyState === WebSocket.CONNECTING) {
    ...
    console.error('WebSocket 正在连接中，请稍后再试')
    return false
  }
  if (readyState === WebSocket.CLOSING) {
    console.log('WebSocket 正在关闭，无法发送消息')
    console.error('WebSocket 正在关闭，无法发送消息')
    return false
  } if (readyState === WebSocket.CLOSED) {
    console.log('WebSocket 已关闭，尝试重新连接')
    console.error('WebSocket 已关闭，尝试重新连接')
    connectWebSocket()
    return false
  }
}
