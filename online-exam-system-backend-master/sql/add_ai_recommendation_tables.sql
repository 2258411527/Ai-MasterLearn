...
CREATE TABLE IF NOT EXISTS `t_ai_daily_recommendation` (
  ...
  `recommendation_type` VARCHAR(50) NOT NULL COMMENT '推荐类型：knowledge_points-知识点 learning_path-学习路径 wrong_analysis-错题分析 book_recommend-图书推荐',
  `ai_content` JSON NOT NULL COMMENT 'AI生成的推荐内容（JSON格式）',
...
CREATE TABLE IF NOT EXISTS `t_ai_knowledge_points` (
  ...
  `priority_level` TINYINT(1) DEFAULT 2 COMMENT '优先级：1-高 2-中 3-低',
...
CREATE TABLE IF NOT EXISTS `t_ai_book_recommendation` (
  ...
  `difficulty_level` TINYINT(1) DEFAULT 2 COMMENT '难度等级：1-简单 2-中等 3-困难',
...
CREATE TABLE IF NOT EXISTS `t_ai_api_log` (
  `id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `user_id` INT(11) NOT NULL COMMENT '用户ID',
  `api_type` VARCHAR(50) NOT NULL COMMENT 'API类型：knowledge-知识点 path-学习路径 wrong-错题分析 book-图书推荐',
