...
CREATE TABLE `t_discussion` (
  ...
  `grade_id` int(11) DEFAULT NULL COMMENT '接收班级id（公开时可为NULL）',
  `content` text NOT NULL COMMENT '内容',
  `title` varchar(255) NOT NULL COMMENT '标题',
  `visibility` tinyint(1) NOT NULL DEFAULT 0 COMMENT '可见范围：0-指定班级可见，1-公开可见',
...
CREATE TABLE `t_user` (
  ...
  `avatar` varchar(255) COLLATE utf8mb4_bin DEFAULT '/uploads/default-avatar.jpg' COMMENT '头像地址',
  `role_id` int(11) DEFAULT '1' COMMENT '角色id',
  `grade_id` int(11) DEFAULT NULL COMMENT '班级id',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间   YYYY-MM-DD hh:mm:ss',
  `status` int(11) DEFAULT '1' COMMENT '状态  1正常0禁用',
  `audit_status` int(11) DEFAULT '1' COMMENT '审核状态：0-待审核，1-审核通过，2-审核拒绝',
  `audit_remark` varchar(500) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '审核备注',
