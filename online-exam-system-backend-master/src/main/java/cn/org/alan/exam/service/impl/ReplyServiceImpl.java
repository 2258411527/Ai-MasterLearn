...
public class ReplyServiceImpl extends ServiceImpl<ReplyMapper, Reply> implements IReplyService {
    ...
    public Integer deleteReply(Integer id) {
        ...
        // 保存讨论ID和是否为顶级评论
        Integer discussionId = reply.getDiscussionId();
        boolean isTopLevel = (reply.getParentId() == -1);
        
        // 删除回复的点赞
        int deleted = likeService.deleteLikeByReplyId(id);
        // 删除回复
        deleted += baseMapper.deleteById(id);
        
        // 如果是顶级评论，更新讨论的评论数
        if (isTopLevel) {
            Discussion discussion = discussionMapper.selectById(discussionId);
            if (discussion != null) {
                discussion.setCommentCount(Math.max(0, (discussion.getCommentCount() == null ? 0 : discussion.getCommentCount()) - 1));
                discussionMapper.updateById(discussion);
            }
        }
