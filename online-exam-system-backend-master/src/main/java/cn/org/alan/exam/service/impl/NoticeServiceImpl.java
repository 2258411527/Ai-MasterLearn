...
public class NoticeServiceImpl extends ServiceImpl<NoticeMapper, Notice> implements INoticeService {
    ...
    public Result<String> addNotice(NoticeForm noticeForm) {
        ...
        userNotificationService.broadcastNotification("notice", notice.getTitle(),
                notice.getContent(), notice.getIsMustRead());
    ...
    public Result<IPage<NoticeVO>> getNewNotice(Integer pageNum, Integer pageSize) {
        ...
        page = noticeMapper.getNewNotice(page, teachIdList, noticeIdList, adminIdList, userId);
        return Result.success("查询成功", page);
    }

    @Override
    public Result<String> markAsRead(Integer noticeId) {
        Integer userId = SecurityUtil.getUserId();
        int row = noticeReadMapper.markAsRead(noticeId, userId);
        if (row > 0) {
    ...
    public Result<Integer> getUnreadCount() {
        Integer userId = SecurityUtil.getUserId();
        Integer gradeId = SecurityUtil.getGradeId();
        int count = noticeReadMapper.countUnread(userId, gradeId);
    ...
    public Result<String> markAllAsRead() {
        ...
        List<Integer> noticeIdList = noticeGradeMapper.getNoticeIdList(gradeId);
        List<Integer> adminIdList = userMapper.getAdminList();
        List<NoticeVO> notices = noticeMapper.getNewNotice(new Page<>(1, Integer.MAX_VALUE), teachIdList, noticeIdList, adminIdList, userId).getRecords();
        for (NoticeVO notice : notices) {
            noticeReadMapper.markAsRead(notice.getId(), userId);
        }
