...
public class DiscussionServiceImpl extends ServiceImpl<DiscussionMapper, Discussion> implements IDiscussionService {
    ...
    public int deleteDiscussion(Integer id) {
        ...
        replyService.deleteByDiscussionId(id);
    ...
    public Page<PageDiscussionVo> pageDiscussionByGrade(String title, Integer currentPage, Integer size) {
        Page<PageDiscussionVo> page = new Page<>(currentPage, size);
        return baseMapper.selectDiscussionByGradePage(page, title, SecurityUtil.getGradeId(), SecurityUtil.getUserId());
    }

    @Override
    public Page<PageDiscussionVo> getHotDiscussions(Integer size) {
        Page<PageDiscussionVo> page = new Page<>(1, size);
        Integer userId = SecurityUtil.getUserId();
        
        // 获取热门话题列表
        List<PageDiscussionVo> hotDiscussions = baseMapper.selectHotDiscussions(userId, size);
        
        // 将列表转换为分页对象
        page.setRecords(hotDiscussions);
        page.setTotal(hotDiscussions.size());
