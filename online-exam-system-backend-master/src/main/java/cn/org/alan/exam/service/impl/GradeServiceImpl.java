...
public class GradeServiceImpl extends ServiceImpl<GradeMapper, Grade> implements IGradeService {
    ...
    public Result<String> deleteGrade(Integer gradeId) {
        examGradeMapper.delete(new LambdaUpdateWrapper<ExamGrade>().eq(ExamGrade::getGradeId, gradeId));

        userGradeMapper.delete(new LambdaUpdateWrapper<UserGrade>().eq(UserGrade::getGId, gradeId));

        userMapper.update(null,
                new LambdaUpdateWrapper<User>().eq(User::getGradeId, gradeId).set(User::getGradeId, null));
    ...
    @Override
    @Transactional
    public Result<String> teacherRemoveStudent(Integer studentId) {
        ...
        if (student.getGradeId() == null) {
            return Result.failed("该学生未加入任何班级");
        }
        List<Integer> teacherGradeIds = userGradeMapper.getGradeIdListByUserId(teacherId);
        if (teacherGradeIds == null || teacherGradeIds.isEmpty() || !teacherGradeIds.contains(student.getGradeId())) {
            return Result.failed("您无权移除该班级的学生");
        }
        userMapper.update(null,
                new LambdaUpdateWrapper<User>().eq(User::getId, studentId).set(User::getGradeId, null));
        userGradeMapper.delete(
                new LambdaUpdateWrapper<UserGrade>().eq(UserGrade::getUId, studentId).eq(UserGrade::getGId, student.getGradeId()));
        return Result.success("已将学生移出班级");
    }
