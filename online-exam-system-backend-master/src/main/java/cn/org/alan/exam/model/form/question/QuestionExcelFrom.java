...
public class QuestionExcelFrom {
    @ExcelImport(value = "试题类型", required = true, kv = "1=单选题,2=多选题,3=判断题,4=简答题")
        ...
        for (QuestionExcelFrom questionExcelFrom : questionExcelFroms) {
            ...
            if (quType == null) {
                throw new ServiceRuntimeException(String.format(
                    "导入错误 - 题干为「%s」的试题：试题类型为空，请填写正确的试题类型（1=单选题 2=多选题 3=判断题 4=简答题）",
                    questionExcelFrom.getContent()));
            }
            if (quType < 1 || quType > 4) {
                throw new ServiceRuntimeException(String.format(
                    "导入错误 - 题干为「%s」的试题：试题类型值%d不合法，请填写1=单选题 2=多选题 3=判断题 4=简答题",
                    questionExcelFrom.getContent(), quType));
            }
