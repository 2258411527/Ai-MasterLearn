...
import cn.org.alan.exam.model.vo.token.record.ExamRecordDetailVO;
...
//                     log.error("无法找到题型 {} 的分数配置, Exam ID: {}", quType, examId);
    ...
    public Result<String> deleteExam(String ids) {
        ...
        for (Integer examId : examIds) {
            examGradeMapper.delete(new LambdaQueryWrapper<ExamGrade>().eq(ExamGrade::getExamId, examId));
            examRepoMapper.delete(new LambdaQueryWrapper<ExamRepo>().eq(ExamRepo::getExamId, examId));
            ...
            List<ExamQuAnswer> answers = examQuAnswerMapper.selectList(
                    new LambdaQueryWrapper<ExamQuAnswer>().eq(ExamQuAnswer::getExamId, examId));
            if (!answers.isEmpty()) {
                List<Integer> answerIds = answers.stream().map(ExamQuAnswer::getId).collect(Collectors.toList());
                userBookMapper.delete(new LambdaQueryWrapper<UserBook>().in(UserBook::getQuId, answerIds));
                examQuAnswerMapper.delete(new LambdaQueryWrapper<ExamQuAnswer>().eq(ExamQuAnswer::getExamId, examId));
            }

            userExamsScoreMapper.delete(new LambdaQueryWrapper<UserExamsScore>().eq(UserExamsScore::getExamId, examId));
        }
    ...
    public Result<ExamQuestionListVO> getQuestionList(Integer examId) {
        Integer userId = SecurityUtil.getUserId();
        // 检查用户是否可以查看题目列表（使用宽松检查）
        if (!canUserViewQuestions(examId)) {
    ...
    public Result<Integer> addCheat(Integer examId) {
        ...
        // 防止maxCount为null导致空指针异常
        Integer maxCount = exam.getMaxCount();
        if (maxCount == null) {
            maxCount = 5; // 设置默认最大切屏次数为5次
            log.warn("考试ID {} 的maxCount为null，使用默认值: {}", examId, maxCount);
        }
        
        // 操作次数，自动交卷
        if (userExamsScore.getCount() >= maxCount) {
        ...
        return Result.success("请勿切屏，最大切屏次数：" + maxCount + ",已切屏次数:" + (userExamsScore.getCount() + 1), 0);
    }

    @Override
    public Result<String> addAnswer(ExamQuAnswerAddForm examQuAnswerForm) {
        // 检查答案长度（简答题可能很长，但需要限制在合理范围内）
        if (examQuAnswerForm.getAnswer() != null && examQuAnswerForm.getAnswer().length() > 5000) {
            return Result.failed("答案长度超过限制（最大5000字符），请适当缩短答案内容");
        }
    ...
    public Result<String> insertNewAnswer(ExamQuAnswerAddForm examQuAnswerForm, Integer quType) {
        ...
        switch (quType) {
            ...
            case 4:
                // 简答题不需要自动判断对错，由教师或AI阅卷
                examQuAnswer.setIsRight(null); // 简答题不设置自动判断结果
                examQuAnswerMapper.insert(examQuAnswer);
                return Result.success("简答题答案保存成功，将由教师或AI阅卷");
            default:
        ...
        switch (quType) {
            ...
            case 4:
                ...
                updateWrapper4.eq(ExamQuAnswer::getUserId, SecurityUtil.getUserId())
                        ...
                        .set(ExamQuAnswer::getIsRight, null); // 简答题不设置自动判断结果
                examQuAnswerMapper.update(null, updateWrapper4);
                return Result.success("简答题答案更新成功，将由教师或AI阅卷");
            default:
                return Result.failed("请求错误，请联系管理员解决");
        }
    }
    ...
    public boolean isUserTakingExam(Integer examId) {
        // 判断是否正在考试 - 严格检查：只有状态为0（正在考试）才算正在进行
    ...
    /**
     * 检查用户是否可以查看题目详情（比isUserTakingExam更宽松）
     */
    public boolean canUserViewQuestions(Integer examId) {
        // 判断是否正在考试 - 简化检查逻辑，只要用户有该考试的记录就认为可以查看题目
        LambdaQueryWrapper<UserExamsScore> userExamsScoreLambdaQueryWrapper = new LambdaQueryWrapper<>();
        userExamsScoreLambdaQueryWrapper.eq(UserExamsScore::getUserId, SecurityUtil.getUserId())
                .eq(UserExamsScore::getExamId, examId);
