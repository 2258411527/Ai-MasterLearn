...
public class DiscussionController {
    ...
    @ApiOperation("获取热门话题")
    @PreAuthorize("hasAnyAuthority('role_teacher','role_student','role_admin')")
    public Result<Page<PageDiscussionVo>> getHotDiscussions(
            @RequestParam(value = "size", required = false, defaultValue = "5") Integer size
    ) {
        Page<PageDiscussionVo> hotDiscussions = discussionService.getHotDiscussions(size);
        return Result.success("查询成功", hotDiscussions);
    }
}