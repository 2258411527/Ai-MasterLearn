...
import cn.org.alan.exam.model.vo.token.record.ExamRecordVO;
import cn.org.alan.exam.model.vo.token.record.ExerciseRecordDetailVO;
...
public class ExerciseRecordServiceImpl extends ServiceImpl<ExerciseRecordMapper, ExerciseRecord>
    ...
    public Result<List<ExamRecordDetailVO>> getExamRecordDetail(Integer examId, Integer userId) {
        ...
        Map<Integer, ExamQuestion> examQuestionMap = examQuestions.stream()
                .collect(Collectors.toMap(ExamQuestion::getQuestionId, eq -> eq, (a, b) -> a));
        ...
        for (Question temp : questions) {
            ...
            examRecordDetailVO.setId(temp.getId());
            ...
            ExamQuestion eq = examQuestionMap.get(temp.getId());
            if (eq != null) {
                examRecordDetailVO.setScore(eq.getScore());
            }
            // 查询试题选项
            ...
            if (examQuAnswer == null) {
                ...
                examRecordDetailVO.setActualScore(0);
                examRecordDetailVOS.add(examRecordDetailVO);
                continue;
            }
            examRecordDetailVO.setAiScore(examQuAnswer.getAiScore());
            examRecordDetailVO.setAiReason(examQuAnswer.getAiReason());
            Integer questionScore = examRecordDetailVO.getScore();
            switch (quType) {
                case 1:
                    ...
                    if (byId1.getIsRight() == 1) {
                        examRecordDetailVO.setIsRight(1);
                        examRecordDetailVO.setActualScore(questionScore);
                    } else {
                ...
                case 2:
                    ...
                    if (examRecordDetailVO.getIsRight() == 1) {
                        examRecordDetailVO.setActualScore(questionScore);
                    } else {
                        examRecordDetailVO.setActualScore(0);
                    }
                ...
                case 4:
                    ...
                    examRecordDetailVO.setIsRight(-1);
                    if (examQuAnswer.getAiScore() != null && examQuAnswer.getAiScore() > 0) {
                        examRecordDetailVO.setActualScore(examQuAnswer.getAiScore());
                    } else {
                        LambdaQueryWrapper<ManualScore> msWrapper = new LambdaQueryWrapper<>();
                        msWrapper.eq(ManualScore::getExamQuAnswerId, examQuAnswer.getId());
                        ManualScore manualScore = manualScoreMapper.selectOne(msWrapper);
                        if (manualScore != null && manualScore.getScore() != null) {
                            examRecordDetailVO.setActualScore(manualScore.getScore());
                        } else {
                            examRecordDetailVO.setActualScore(null);
                        }
                    }
                    break;
                default:
